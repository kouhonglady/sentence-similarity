# -*- coding: utf-8 -*-
"""
Created on Mon Jul 30 09:14:07 2018
...
#X_train = pd.read_csv(train_feature_path,header = None)
...
#Y = OneHotEncoder().fit_transform(np.array(y_train).reshape(-1,1)).todense()
#X = np.array(X_train).reshape(-1,15,15,1)
#
#
...
#xs = tf.placeholder(tf.float32, [None, 15*15]) 
...
#     for batch in generatebatch(X,Y,Y.shape[0],50): # 每个周期进行MBGD算法
#          print(batch[0].shape)
#          print(batch[1].shape)
...
#
#    
#print("test accuracy %g"%accuracy.eval(feed_dict={xs: X, ys: Y, keep_prob: 1.0}))



from tensorflow.examples.tutorials.mnist import input_data
import tensorflow as tf
mnist = input_data.read_data_sets(".\dataset", one_hot=True)# 读取图片数据集
sess = tf.InteractiveSession()# 创建session
# 一，函数声明部分
